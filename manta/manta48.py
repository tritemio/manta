...
class PixelGroup(object):
    ...
    @property
    def shape(self):
        """Group shape in thte case is specified through slices.
        """
        if self._slices is not None:
...
class Manta48(object):
    ...
    def __init__(self, index=False, lcos_coord=True, horiz=False,
        ...
        if horiz:
            self.left_half = self.spad[:, :self.nrows/2]
            self.right_half = self.spad[:, self.nrows/2:]
        else:
            self.top_half = self.spad[:self.nrows/2]
            self.bot_half = self.spad[self.nrows/2:]
